          ...
          {activeTab === 'dashboard' ? (
            ...
            <div className="max-w-4xl mx-auto amoled-vault-panel p-6 relative">
              ...
              <div className="relative z-10">
                ...
                <button onClick={() => setActiveTab('dashboard')} className="px-4 py-2 glass-pill text-xs text-slate-300 font-bold transition-all">Dashboard Hub</button>
              </div>
              ...
              {!isLoggedIn ? (
                ...
                <div className="text-center py-12 flex flex-col items-center justify-center">
                  ...
                  <button onClick={unlockVault} disabled={vaultLoading} className="px-5 py-2.5 glass-pill text-white text-xs font-bold transition-all flex items-center gap-2 disabled:opacity-50">
                    {vaultLoading ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Unlock className="w-3.5 h-3.5 text-white" />}
                    <span>Biometric Authorization Lockout Challenge</span>
                  </button>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="flex justify-between items-center glass-pill px-4 py-3 text-[10px]">
                    <div>
                      <span className="text-slate-400">Decryption Matrix Status: </span>
                      <span className="font-bold text-white uppercase">Active in Server Volatile Memory Pools</span>
                    </div>
                    {clipboardTTL > 0 && (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    <div className="md:col-span-6 space-y-3">
                      ...
                      <div className="max-h-80 overflow-y-auto pr-1 scroll-dark space-y-2.5">
                        ...
                        {vaultSecrets.map((secret) => (
                          <div key={secret.id} className="secret-row">
                            ...
                            <div className="flex items-center gap-1 shrink-0 glass-pill px-2 py-1">
                              <button onClick={() => toggleReveal(secret.id)} className="p-1.5 rounded-full text-slate-400 hover:text-white transition-colors">
                                {revealedIds.has(secret.id) ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                              </button>
                              <div className="w-px h-3 bg-white/10 mx-0.5"></div>
                              <button onClick={() => copyToClipboard(secret.value)} className="p-1.5 rounded-full text-slate-400 hover:text-white transition-colors">
                                <Copy className="w-3.5 h-3.5" />
                              </button>
                              <div className="w-px h-3 bg-white/10 mx-0.5"></div>
                              <button onClick={() => deleteSecretById(secret.id)} className="p-1.5 rounded-full text-slate-400 hover:text-white transition-colors">
                      ...
                      {!showAddSecret && (
                        <button onClick={() => setShowAddSecret(true)} className="w-full py-2.5 rounded-xl border border-dashed border-white/20 text-xs text-slate-400 hover:text-white hover:border-white/50 transition-all flex items-center justify-center gap-1.5">
                    ...
                    <div className="md:col-span-6 space-y-4">
                      {showAddSecret ? (
                        <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                          ...
                          <input value={newSecretLabel} onChange={(e) => setNewSecretLabel(e.target.value)} placeholder="Secret Label (e.g. AWS Token)" className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-white/40" />
                          
                          <div className="flex gap-2">
                            <input value={newSecretValue} onChange={(e) => { setNewSecretValue(e.target.value); setAuditResult(''); }} placeholder="Plaintext secret value" type="password" className="flex-grow bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-white/40" />
                            <button onClick={auditPassword} disabled={isAuditing} className="px-4 py-2 glass-pill text-white text-xs font-bold disabled:opacity-50">
                          ...
                          {auditResult && (
                            <div className="text-[10px] bg-black/40 border border-white/10 p-3 rounded-xl text-slate-300 leading-normal flex flex-col gap-1.5">
                              <span className="font-bold text-white border-b border-white/10 pb-1">AI Audit Report:</span>
                          ...
                          <div className="border-t border-white/5 pt-3">
                            {showPwGenDrawer ? (
                              <div className="p-3 rounded-xl bg-black/40 border border-white/10 space-y-3">
                                <div className="text-[9px] text-white font-bold uppercase tracking-wider font-mono">AI Mnemonic Generator</div>
                                <div className="flex flex-wrap gap-1.5">
                                  <button type="button" onClick={() => setPwGenPrompt('Strong password for a secure web portal')} className="px-2 py-0.5 rounded glass-pill text-[8px] text-slate-300 transition-all">+ Web Portal</button>
                                  <button type="button" onClick={() => setPwGenPrompt('High entropy random API credentials')} className="px-2 py-0.5 rounded glass-pill text-[8px] text-slate-300 transition-all">+ API Token</button>
                                  <button type="button" onClick={() => setPwGenPrompt('An easy to remember mnemonic pass phrase')} className="px-2 py-0.5 rounded glass-pill text-[8px] text-slate-300 transition-all">+ Mnemonic</button>
                                </div>
                                <div className="flex gap-2">
                                  <input value={pwGenPrompt} onChange={(e) => setPwGenPrompt(e.target.value)} placeholder="Describe password purpose..." className="flex-grow bg-black/40 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none focus:border-white/40" />
                                  <button onClick={generateMnemonicPassword} disabled={isGeneratingPw} className="px-3 py-1 glass-pill text-white text-xs font-bold disabled:opacity-50">Generate</button>
                                </div>
                              ...
                              <button type="button" onClick={() => setShowPwGenDrawer(true)} className="text-[9px] text-slate-400 hover:text-white hover:underline flex items-center gap-1 font-mono">
                          ...
                          <div className="flex gap-2 pt-2">
                            <button onClick={addSecret} className="flex-grow py-2 glass-pill text-white text-xs font-bold transition-all">Encrypt & Store</button>
                  ...
                  <div className="flex justify-between border-t border-white/5 pt-4">
                    <button onClick={purgeVault} className="px-5 py-2.5 glass-pill text-white text-xs font-bold transition-all">Lock Vault & Purge RAM</button>
                  </div>
                </div>
              )}
              </div>
